...
- Upload `index.html`, `config.js` (with your real credentials filled in), and
  `schema.sql` / `README.md` if you like — either via the GitHub web UI ("Add file →
  Upload files") or `git push` from Terminal if you're comfortable with git.
...
- Build settings: leave **Build command** blank and set **Build output directory** to
